...
export function createHud(container, getState, camera) {
  ...
  `;
	...
	for (const btn of container.querySelectorAll(".hud-btn"))
		attachButtonSounds(btn);
	...
	btnClose.addEventListener("click", () =>
		els.upgradePanel.classList.remove("open"),
	);
	...
	window.addEventListener("keydown", (e) => {
		...
		if (idx < UPGRADE_DEFS.length)
			els.upgradeBtns[UPGRADE_DEFS[idx].id]?.click();
	});
...
export function updateHud(hud, state, camera, onRestart, onQuit, onResume) {
	if (!hud._listenersAttached) {
		...
		document
			.getElementById("btn-restart-gameover")
			?.addEventListener("click", onRestart);
		document
			.getElementById("btn-mainmenu-gameover")
			?.addEventListener("click", onQuit);
		document.getElementById("btn-resume")?.addEventListener("click", onResume);
		document
			.getElementById("btn-mainmenu-pause")
			?.addEventListener("click", onQuit);
	}

	// Modals
	hud.modalGameover.classList.toggle(
		"visible",
	...
	if (state.wavePhase === "buildup") {
		...
		hud.wave.textContent =
			wave === 0 ? `PREPARE — T-${secs}s` : `${label} — T-${secs}s`;
	...
	hud.hpFill.style.background =
		pct > 0.5 ? "var(--accent)" : pct > 0.25 ? "var(--gold)" : "var(--danger)";
