...
function drawChevron(ctx, x, y, size, heading, color, scaleX = 1) {
...
function drawMiniboss(ctx, x, y, size, heading, color, scaleX = 1) {
...
function drawBuildings(ctx, buildings) {
	...
	for (const b of buildings) {
		...
		switch (b.type) {
			case "extractor":
				drawExtractor(ctx, b.x, b.y, "#00d4ff");
				break;
			case "lightTurret":
				drawLightTurret(ctx, b.x, b.y, "#00d4ff", b.heading);
				break;
			case "turretPlatform":
				drawTurretPlatform(ctx, b.x, b.y, "#00d4ff", b.heading);
				break;
			case "shipyard":
				drawShipyard(ctx, b.x, b.y, "#00d4ff");
				break;
			case "cryoBattery":
				drawCryoBattery(ctx, b.x, b.y, "#a8d2e8");
				break;
			case "fortress":
				drawFortress(ctx, b.x, b.y, "#00d4ff");
				break;
		}
		ctx.restore();
	}
...
function drawEnemy(ctx, e) {
	const anim = e.spawnAnim ?? 0;
	const t = anim > 0 ? Math.min(1, anim / 0.2) : 0;
	ctx.save();
	if (t > 0) ctx.globalAlpha = 1 - t;
	const scaleX = 1 + 9 * t;
	switch (e.type) {
		case "skirmisher":
			drawTriangle(ctx, e.x, e.y, 10, e.heading, "#ff8c00", 1.2, scaleX);
			break;
		case "bomber":
			drawChevron(ctx, e.x, e.y, 13, e.heading, "#ff8c00", scaleX);
			break;
		case "miniboss":
			drawMiniboss(ctx, e.x, e.y, 22, e.heading, "#ff8c00", scaleX);
			break;
	}
	ctx.restore();
}
...
function clampToEdge(sx, sy, W, H, margin) {
	...
	const scale = Math.min(
		inW / (Math.abs(dx) || 0.001),
		inH / (Math.abs(dy) || 0.001),
	);
	const clamped =
...
function drawWaveOriginIndicators(ctx, state, camera, W, H) {
	...
	for (const origin of state.waveOrigins) {
		...
		const alpha = isBuildup ? 0.45 + 0.4 * Math.sin(t * 5) : 0.85;
